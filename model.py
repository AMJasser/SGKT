...
class SGKT(object):
    def __init__(self, args):
        ...
        self.features_answer_index = tf.placeholder(
            tf.int32, [None, self.max_step + 1, self.field_size])
        self.target_answers = tf.placeholder(tf.float32, [None, self.max_step])
        self.sequence_lens = tf.placeholder(tf.int32, [None])
        self.hist_neighbor_index = tf.placeholder(
            tf.int32, [None, self.max_step, self.hist_neighbor_num])
        self.batch_size = tf.shape(self.features_answer_index)[0]
        self.feature_embedding = tf.get_variable("feature_embedding", [
                                                 self.feature_answer_size, self.embedding_size], initializer=tf.keras.initializers.glorot_normal())
        ...
        self.nasr_v = tf.get_variable('nasr_v', [self.embedding_size, self.embedding_size], dtype=tf.float32,
                                      initializer=tf.random_uniform_initializer(-self.stdv, self.stdv))
        self.nasr_b = tf.get_variable('nasr_b', [
                                      self.embedding_size], dtype=tf.float32, initializer=tf.zeros_initializer())
    ...
    def build_model(self):
        ...
        select_feature_index = tf.gather(
            self.features_answer_index, self.select_index, axis=-1)
        ...
        self.input_questions_embedding = tf.nn.embedding_lookup(
            self.feature_embedding, questions_index)  # [batch_size,seq_len,d]
        self.next_questions_embedding = tf.nn.embedding_lookup(
            self.feature_embedding, next_questions_index)  # [batch_size,seq_len,select_size-1,d]
        self.input_skills_embedding = tf.nn.embedding_lookup(
            self.feature_embedding, skill_index)  # [batch_size,seq_len,d]
        self.next_skills_embedding = tf.nn.embedding_lookup(
            self.feature_embedding, next_skill_index)  # [batch_size,seq_len,select_size-1,d]
        input_answers_embedding = tf.nn.embedding_lookup(
            self.feature_embedding, input_answers_index)  # [batch_size,seq_len,1,d]
        ...
        if self.n_hop > 0:
            ...
            aggregate_embedding, self.aggregators = self.aggregate(
                input_neighbors, self.input_questions_embedding)
            # [[batch_size,seq_len],[batch_size,seq_len,q_neighbor_num],[batch_size,seq_len,q_neighbor_num*s_neighbor_num]
            next_input_neighbors = self.get_neighbors(
                self.n_hop, next_questions_index)
            next_aggregate_embedding, self.aggregators = self.aggregate(
                next_input_neighbors, self.next_questions_embedding)
            feature_emb_size = self.embedding_size
            feature_trans_embedding = tf.reshape(tf.layers.dense(tf.reshape(
                aggregate_embedding[0], [-1, feature_emb_size]), hidden_size, activation=tf.nn.relu, name='feature_layer', reuse=False), [-1, self.max_step, hidden_size])  # [batch_size,max_step,hidden_size]
            next_trans_embedding = tf.reshape(tf.layers.dense(tf.reshape(next_aggregate_embedding[0], [
                                              -1, feature_emb_size]), hidden_size, activation=tf.nn.relu, name='feature_layer', reuse=True), [-1, self.max_step, hidden_size])  # [batch_size,max_step,hidden_size]
        else:
            feature_emb_size = self.embedding_size
            feature_trans_embedding = tf.reshape(tf.layers.dense(tf.reshape(self.input_questions_embedding, [
                                                 -1, feature_emb_size]), hidden_size, activation=tf.nn.relu, name='feature_layer', reuse=False), [-1, self.max_step, hidden_size])  # [batch_size,max_step,hidden_size]
            next_trans_embedding = tf.reshape(tf.layers.dense(tf.reshape(self.next_questions_embedding, [
                                              -1, feature_emb_size]), hidden_size, activation=tf.nn.relu, name='feature_layer', reuse=True), [-1, self.max_step, hidden_size])  # [batch_size,max_step,hidden_size]
        ...
        input_fa_embedding = tf.reshape(tf.concat(
            [feature_trans_embedding, input_answers_embedding], -1), [-1, hidden_size+self.embedding_size])  # embedding_size*2
        ...
        fin_state = tf.nn.embedding_lookup(
            self.feature_embedding, input_answers_index[:, 0])
        cell = tf.nn.rnn_cell.GRUCell(self.embedding_size, dtype=tf.float32)
        with tf.variable_scope('gru'):
            for i in range(self.max_step):
                question_state = tf.nn.embedding_lookup(
                    self.feature_embedding, questions_index[:, i])
                answer_state = tf.nn.embedding_lookup(
                    self.feature_embedding, input_answers_index[:, i])
                in_state = question_state + answer_state + \
                    input_trans_embedding[:, i, :]
                fin_state_in = tf.reshape(
                    tf.matmul(in_state, self.W_in) + self.b_in, [-1, self.embedding_size])
                fin_state_out = tf.reshape(
                    tf.matmul(in_state, self.W_out) + self.b_out, [-1, self.embedding_size])
            ...
            output_series = tf.reshape(
                tf.concat(output_series, 1), [-1, self.max_step, self.embedding_size])
        ...
        if self.hist_neighbor_num != 0:
            ...
            # [self.batch_size,max_step,1,feature_trans_size]
            Nh = tf.expand_dims(output_series, 2)
            logits = tf.reduce_sum(tf.expand_dims(Nh, 3) * tf.expand_dims(Nn, 2),
                                   axis=4)  # [-1,max_step,Nh,1,emb_size]*[-1,max_step,1,Nn,emb_size]
            logits = tf.reshape(logits,
                                [-1, self.max_step, 1 * next_neighbor_num])  # ====>[batch_size,max_step,Nu*Nv]
        with tf.variable_scope('ni'):
            w1 = tf.get_variable('atn_weights_1', [
                                 hidden_size, 1], initializer=tf.keras.initializers.glorot_normal())
            w2 = tf.get_variable('atn_weights_2', [
                                 hidden_size, 1], initializer=tf.keras.initializers.glorot_normal())
            b1 = tf.get_variable(
                'atn_bias_1', [1], initializer=tf.zeros_initializer())
            b2 = tf.get_variable(
                'atn_bias_2', [1], initializer=tf.zeros_initializer())
        if select_size > 3:
        ...
        self.flat_target_logits = flat_target_logits = tf.reshape(
            self.logits, [-1])
        self.flat_target_correctness = tf.reshape(self.target_answers, [-1])
        self.pred = tf.sigmoid(tf.reshape(
            flat_target_logits, [-1, self.max_step]))
        ...
        index = tf.where(tf.not_equal(tf.reshape(
            self.filling_seqs, [-1]), tf.constant(0, dtype=tf.float32)))
        clear_flat_target_logits = tf.gather(self.flat_target_logits, index)
        clear_flat_target_correctness = tf.gather(
            self.flat_target_correctness, index)
        ...
        self.grads, _ = tf.clip_by_global_norm(
            tf.gradients(self.loss, trainable_vars), 50)
    ...
    def hist_neighbor_sampler(self, input_embedding):
        ...
        for i in range(self.hidden_neurons[-1]):
            temp.append(tf.matmul(tf.exp(
                tf.matmul((input_embedding[:, :, i]-input_embedding[:, :, 0]), xt1)), xt2)+x)
        input_embedding = tf.reshape(
            tf.stack(temp), [self.batch_size, self.max_step, self.hidden_neurons[-1]])
        zero_embeddings = tf.expand_dims(tf.zeros(
            [self.batch_size, self.hidden_neurons[-1]], dtype=tf.float32), 1)  # [batch_size,1,hidden_size]
        # [batch_size,max_step+1,hidden_size]
        input_embedding = tf.concat([input_embedding, zero_embeddings], 1)
        ...
        A = tf.matmul(EQ, EK, transpose_b=True) / \
            tf.sqrt(float(self.hidden_neurons[-1]))
        O = tf.matmul(A, EV)
        # [self.batch_size, max_step*M]
        temp_hist_index = tf.reshape(
            self.hist_neighbor_index, [-1, self.max_step*self.hist_neighbor_num])
        hist_neighbors_features = tf.reshape(tf.batch_gather(
            O, temp_hist_index), [-1, self.max_step, self.hist_neighbor_num, input_embedding.shape[-1]])
        return hist_neighbors_features

    # sample based on question similarity
    def hist_neighbor_sampler1(self, input_q_emb, next_q_emb, qa_emb):
        # next_q_emb:[batch_size,ms,emb_size]
        mold_nextq = tf.sqrt(tf.reduce_sum(
            next_q_emb*next_q_emb, -1))  # [bs,ms]
        next_q_emb = tf.expand_dims(next_q_emb, 2)
        mold_inputq = tf.sqrt(tf.reduce_sum(
            input_q_emb*input_q_emb, -1))  # [bs,ms]
        input_q_emb = tf.expand_dims(input_q_emb, 1)
        q_similarity = tf.reduce_sum(
            next_q_emb*input_q_emb, -1)  # [batch_size,ms,ms]
        molds = tf.expand_dims(mold_nextq, 2) * \
            tf.expand_dims(mold_inputq, 1)  # [bs,ms,ms]
        q_similarity = q_similarity/molds
        zero_embeddings = tf.expand_dims(tf.zeros(
            [self.batch_size, self.hidden_neurons[-1]], dtype=tf.float32), 1)  # [batch_size,1,hidden_size]
        # [batch_size,max_step+1,hidden_size]
        qa_emb = tf.concat([qa_emb, zero_embeddings], 1)
        ...
        # only history q non zero# [batch_size,ms,ms]
        ...
        q_similarity = tf.where(condition, q_similarity, tf.zeros(
            [self.batch_size, self.max_step, self.max_step]))
        self.q_similarity = q_similarity
        temp_hist_index = tf.nn.top_k(q_similarity, self.hist_neighbor_num)[
            1]  # [batch_size,ms,hist_num]
        self.hist_attention_value = tf.nn.top_k(q_similarity, self.hist_neighbor_num)[
            0]  # [batch_size,ms,hist_num]
        # q_similarity[temp_hist_index]>0
        temp_hist_index = tf.where(
            self.hist_attention_value > 0, temp_hist_index, self.hist_neighbor_index)
        temp_hist_index = tf.reshape(
            temp_hist_index, [-1, self.max_step*self.hist_neighbor_num])
        hist_neighbors_features = tf.reshape(tf.batch_gather(
            qa_emb, temp_hist_index), [-1, self.max_step, self.hist_neighbor_num, qa_emb.shape[-1]])
        return hist_neighbors_features

    def next_neighbor_sampler(self, aggregate_embedding):
        temp_emb = tf.reshape(
            aggregate_embedding[1], [-1, self.question_neighbor_num, self.embedding_size])
        ...
        if self.question_neighbor_num >= self.next_neighbor_num:
            next_neighbors_embedding = tf.reshape(temp_emb[:, :self.next_neighbor_num, :], [
                                                  self.batch_size, self.max_step, self.next_neighbor_num, self.embedding_size])
        else:
            tile_neighbor_embedding = tf.tile(
                temp_emb, [1, -(-self.next_neighbor_num // tf.shape(temp_emb)[0]), 1])
    ...
    def get_neighbors(self, n_hop, question_index):
        ...
        # [[batch_size,seq_len],[batch_size,seq_len,question_neighbor_num],batch_size,seq_len,question_neighbor_num,
